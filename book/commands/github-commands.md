...
* `git reset <commit>` - reset reverts changes by moving a branch reference back to an older commit but this only works for local changes
  ...
  * `git reset --hard HEAD~3` - resets the index and the working tree. Any changes to to tracked files in the working tree are discarded.

  * `git reset --soft` - does not touch the index file or the working tree (but resets the head). Tis leaved all your changed files as 'changes to be committed'.
